...
export default function AdminUsers() {
  ...
  const [accounts, setAccounts] = useState<{ id: string; account_number: string; account_type: string; balance: number }[]>([]);
  ...
  const openUser = async (p: Profile) => {
    setOpen(p); setReason(p.kyc_reason ?? "");
    const { data } = await supabase.from("accounts").select("*").eq("user_id", p.id);
    setAccounts(data ?? []);
    if (data && data[0]) setAdjustAcc(data[0].id);
  };
                ...
                {accounts.length > 0 && (
                  <div className="border-t pt-3 space-y-2">
                    ...
                    <select className="w-full border rounded p-2 text-sm bg-background" value={adjustAcc} onChange={(e) => setAdjustAcc(e.target.value)}>
                      {accounts.map((a) => <option key={a.id} value={a.id}>{a.account_type} •••{a.account_number.slice(-4)} (${a.balance})</option>)}
                    </select>
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => creditDebit(1)}>Credit</Button>
                      <Button size="sm" variant="destructive" onClick={() => creditDebit(-1)}>Debit</Button>
                    </div>
                  </div>
                )}
              </DialogContent>
